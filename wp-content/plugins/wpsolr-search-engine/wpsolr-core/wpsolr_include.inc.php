...
// Definitions
define( 'WPSOLR_PLUGIN_VERSION', '18.1' );
...
require_once 'classes/engines/solarium/wpsolr-index-solr-client.php';
require_once 'classes/engines/solarium/wpsolr-search-solr-client.php';

/* Include Elastica clients */
require_once 'classes/engines/elastica/wpsolr-index-elastica-client.php';
require_once 'classes/engines/elastica/wpsolr-search-elastica-client.php';
...
function add_remove_document_to_solr_index( $post_id, $post ) {
	...
	try {
		...
		if ( in_array( $post->post_status, $index_post_statuses, true ) ) {
			...
			$solr = WPSolrAbstractIndexClient::create_from_post( $post );
...
function add_attachment_to_solr_index( $attachment_id ) {
	...
	try {
		$solr = WPSolrAbstractIndexClient::create();
...
function wpsolr_enqueue_script() {
	...
	$is_ajax                   = WPSOLR_Global::getOption()->get_search_is_use_current_theme_with_ajax();
	$container_page_title      = WPSOLR_Global::getOption()->get_option_theme_ajax_page_title_jquery_selectors();
	$container_page_sort       = WPSOLR_Global::getOption()->get_option_theme_ajax_sort_jquery_selectors();
	$container_results         = WPSOLR_Global::getOption()->get_option_theme_ajax_results_jquery_selectors();
	$container_pagination      = WPSOLR_Global::getOption()->get_option_theme_ajax_pagination_jquery_selectors();
	$container_pagination_page = WPSOLR_Global::getOption()->get_option_theme_ajax_pagination_page_jquery_selectors();
	$container_results_count   = WPSOLR_Global::getOption()->get_option_theme_ajax_results_count_jquery_selectors();
			...
			[
				'data' =>
					[
						...
						'is_show_url_parameters'             => WPSOLR_Global::getOption()->get_search_is_show_url_parameters(),
						...
						'ajax_delay_ms'                      => WPSOLR_Global::getOption()->get_option_theme_ajax_delay_ms(),
					],
			]
		),
	 ...
	 */
	if ( WPSOLR_Global::getOption()->get_search_is_infinitescroll() && ! WPSOLR_Global::getOption()->get_search_is_infinitescroll_replace_js() ) {
		// Get localization options
		$localization_options = OptionLocalization::get_options();
