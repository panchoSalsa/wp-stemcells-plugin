...
jQuery(document).ready(function () {
    ...
    // Remove empty fields on saved form.
    jQuery(document).on('submit', '#fac_settings_form', function (e) {
        remove_empty_values();
        remove_hidden_values();
    });

    // Collapse/uncollapse sections
    jQuery(document).on('click', '.wpsolr_collapser', function (e) {
    ...
    jQuery('#check_index_status').click(function () {
        ...
        if (name == '') {
            jQuery('.name_err').text('Please enter a WPSOLR index name');
            error = 1;
        }
        else {
