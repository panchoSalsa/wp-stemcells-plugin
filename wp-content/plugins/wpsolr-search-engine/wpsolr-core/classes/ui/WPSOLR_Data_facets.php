...
class WPSOLR_Data_Facets {
	...
	public static function get_data( $facets_selected, $facets_to_display, $facets_in_results ) {
		...
		if ( count( $facets_in_results ) && count( $facets_to_display ) ) {
			...
			$facets_grids  = WPSOLR_Global::getOption()->get_facets_grid();
		...
		foreach ( $items as $item ) {
			...
			if ( empty( $result[ $item_top_level_name ] ) ) {
				...
				$result[ $item_top_level_name ]          = [
					...
					'value_localized' => self::get_field_value_localization( $facet_to_display_id, $item_top_level_name, null ),
					'count'           => $item['count'],
	 ...
	 */
	public
	static function get_field_value_localization(
