...
class OptionLicenses extends WpSolrExtensions {
	...
	function get_license( $license_type ) {
		...
		return isset( $licenses[ $license_type ] ) ? $licenses[ $license_type ] : array();
	}
	...
	static function get_license_types() {
		...
		return array(
			...
			self::LICENSE_PACKAGE_THEME               => array(
	 ...
	 * @return array
	 */
	public static function get_activated_licenses_titles( $license_type = null ) {
		...
		$licenses        = empty( $license_type ) ? $option_licenses->get_licenses() : array( $license_type => $option_licenses->get_license( $license_type ) );
		...
		foreach ( $licenses as $license_code => $license ) {
			...
			if ( $option_licenses->get_license_is_activated( $license_code ) ) {
				...
				if ( isset( $license_types[ $license_code ] )
				     ...
				     && WpSolrExtensions::is_extension_option_activate( $license_types[ $license_code ][ self::LICENSE_EXTENSION ] )
				) {
