...
class WpSolrExtensions {
    ...
    */
	...
	private static $extensions_array = array(
		...
		self::OPTION_THEME                  =>
			array(
				self::_CONFIG_EXTENSION_CLASS_NAME              => 'OptionTheme',
				self::_CONFIG_PLUGIN_IS_AUTO_ACTIVE             => true,
				self::_CONFIG_EXTENSION_DIRECTORY               => 'theme/',
				self::_CONFIG_EXTENSION_FILE_PATH               => 'theme/option-theme.php',
	...
	static function require_once_wpsolr_extension(
		...
		if ( $is_admin ) {
			// Called from admin: we active the extension, whatever.
			return self::load_file( $extension_config_array[ self::_CONFIG_EXTENSION_FILE_PATH ] );
		}
		...
		$result = self::load_file( $extension_config_array[ self::_CONFIG_EXTENSION_FILE_PATH ] );
	...
	static public function load_file( $file, $is_admin_option = false, $vars = null ) {
		...
		if ( defined( 'WPSOLR_PLUGIN_PRO_DIR' ) && file_exists( WPSOLR_PLUGIN_PRO_DIR . '/classes/extensions/' . $file ) ) {
			require_once( WPSOLR_PLUGIN_PRO_DIR . '/classes/extensions/' . $file );
	...
	static function get_option_file(
		...
		if ( defined( 'WPSOLR_PLUGIN_PRO_DIR' ) && file_exists( WPSOLR_PLUGIN_PRO_DIR . '/classes/extensions/' . $file ) ) {
			$file = WPSOLR_PLUGIN_PRO_DIR . '/classes/extensions/' . $file;
		} else {
