...
class WpSolrSchema {
	...
	public static function get_sort_fields() {
		...
		foreach ( WPSOLR_Global::getOption()->get_option_index_custom_field_properties() as $custom_field_name => $custom_field_property ) {
			...
			if ( self::get_solr_dynamic_entension_id_is_sortable( $custom_field_property[ WPSOLR_Option::OPTION_INDEX_CUSTOM_FIELD_PROPERTY_SOLR_TYPE ] ) ) {
				...
				foreach (
					array(
						array( self::SORT_ASC, 'ascending' ),
						array( self::SORT_DESC, 'descending' ),
					) as $sort_order
				) {
					$custom_fields_sortable[] = array(
	 ...
	 *
	 * @param WPSolrAbstractIndexClient $search_engine_client
	 * @param string $field_name
	 * @param string $value
	 * @param string $field_type
	 *
	...
	function get_sanitized_float_value(
		WPSolrAbstractIndexClient $search_engine_client, $field_name, $value, $field_type, $post
	) {
	 ...
	 * @throws Exception
	 */
	public static function get_sanitized_value( WPSolrAbstractIndexClient $search_engine_client, $field_name, $value, $post ) {
