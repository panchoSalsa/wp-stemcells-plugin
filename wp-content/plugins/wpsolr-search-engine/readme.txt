=== Search 10 times faster with Elasticsearch or Apache Solr with lots of data - WPSOLR ===
...
Search 10 times faster with Elasticsearch or Apache Solr with lots of data. Suggestions, attachments, boosts, Ajax, facets, multilingual, synonyms ...

== Description ==

Free <a href="https://www.wpsolr.com/?org=wordpress&cmp=chat">Zendesk chat assistance</a> to help you start using the free plugin fast.

Replace your Wordpress or WooCommerce search with Elasticsearch or Apache Solr. See it by yourself on our <a href="https://www.wpsolr.com/?s=">website search</a>.

Video portfolio of sites search and product filter with 100Ks products:
[youtube https://www.youtube.com/playlist?list=PL5aStiCXsx-yHhZ7qixtpSszCcXZTQUyW]

And no more excuses for not trying the plugin: a <a href="https://www.wpsolr.com/guide/configuration-step-by-step-schematic/configure-your-indexes/configure-a-temporary-apache-solr-index-for-testing/">free hosted Solr index</a> is provided with the plugin, for 2 hours. Just a few clicks, and your thousands of posts or products can be indexed and searched.
After that, if you are convinced, you can spend time on installing your own Solr index.

Here are the few steps to manage as many posts or products as you want (thousands to millions):
> * <a href="https://www.wpsolr.com/guide/configuration-step-by-step-schematic/install-elasticsearch/">Install Elasticsearch</a>, or <a href="https://www.wpsolr.com/guide/configuration-step-by-step-schematic/install-apache-solr/">install Apache Solr</a>
> * For Elasticsearch: the plugin will create and setup your index. For Apache Solr: <a href="https://www.wpsolr.com/guide/configuration-step-by-step-schematic/configure-your-indexes/create-configure-apache-solr-index/">create your index</a>, then copy the <a href="https://www.wpsolr.com/kb/apache-solr/apache-solr-configuration-files/">configuration files</a> and reload the index.
> * Select the data you want to index
> * Select how the search will work and be displayed
> * Index your data
> * Replace your search with your new search

The plugin is free for the following features, with no limitation on number of searches or documents:
> * Manage one index (Elasticsearch or Apache)
> * Index posts, pages, products, comments. Exclude some by ids
> * Use post type, tags, categories, authors as facets (filters)
> * Hard-coded Ajax search template with: sort, filters (facets), results
> * Keyword suggestions while you type
> * Show snippets on results (texts containing keywords highlighted)
> * Use partial keyword matches in results
> * Use fuzzy keyword matches in results
> * Choose and reorder the sort list items: more relevent, newest, oldest, more comments, less comments
> * Change all labels on the search page
> * Index by batch and real-time (as soon as a post is saved)

For websites with more needs, there is a WPSOLR PRO version available at <a href="https://www.wpsolr.com/">wpsolr.com</a>.

WPSOLR PRO adds extensions and Zendesk support:
> * Theme: collapse facet hierarchies, customize your facets with a bit of css, select facets layouts for checkboxes or radioboxes
> * Premium: index and search attachments in the media library, manage more than one Solr index, multi-domain search, use your theme's search template instead of WPSOLR's, change ajax search page slug, infinite scroll navigation, products suggestions, attach suggestions to any search box with jQuery selectors,
id you mean?, stop real-time indexing, index/search any post type, index/search custom taxonomies, index/search custom fields, index/search attached files from the medial library,
boost fields, localize facets, show facets hierarchies, localize sort items, display debug infos during indexing, re-index all the data in place
> * Geolocation: sort results by distance to the user's location, show distance in each search result, trigger geolocation on any search box, filter out results with no geolocation
> * WooCommerce: index and search and sort by product attributes/variations/sku/price, filter by catalogo visibility, replace WooCommerce orders search, replace WooCommerce drop-down list sort content with any custom field/attribute, replace WooCommerce category and shop search
> * Advanced Custom Fields (ACF): index and search ACF fields multi-select/files/flexible layout, use ACF google map field with the Geolocation extension
> * Toolset types: replace custom field name by TYPES custom field label on facets
> * WPML: index and search languages in their own index, translate facet labels and contents with WPML string module
> * Polylang: index and search languages in their own index, translate facet labels and contents with Polylang string module
> * TablePress: index and search tables content
> * Groups: filter search results with user's security (Solr only)
> * s2Member: filter search results with user's security (Solr only)
> * bbPress: replace bbPress search transparently
> * Embed Any Document: index and search documents embedded
> * PDF Embedder: index and search documents embedded
> * Google Doc Embedder: index and search documents embedded

WPSOLR and WPSOLR PRO are built with WordPress filters and actions: you can pretty much do anything you want with them.
